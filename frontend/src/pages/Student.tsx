  ...
  useEffect(() => {
    async function loadData() {
      if (selectedClass) {
        loadTests();
        try {
          const materialsResp = await api.get(`/materials/class/${selectedClass.id}`);
        ...
        if (!studentId) return;
        fetch(`http://localhost:4000/api/tests/history/${studentId}`)
          .then(res => res.json())
          .then(data => {
            console.log(data);
            setAttempts([...data]);
...
handleNextQuestion(); return;
  ...
  async function startTest(testId: string) {
    try {
      ...
      setActiveTest({ questions: resp.data.questions });
...
function handleNextQuestion() {
  ...
  async function submitAnswer() {
    ...
    const stress = Math.random() * 0.3 + (isCorrect ? 0 : 0.4);
    setAnswers([...answers, { isCorrect, stress, time: timeTaken }]);
    try {
      console.log('Debug:', { activeTestId, attemptId, questionId: currentQuestion.id || currentQuestion._id });
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div className="card text-center">
              ...
              <p className="text-4xl font-bold text-blue-600 mt-2">{(insights.accuracy * 100).toFixed(0)}%</p>
            </div>
            <div className="card text-center">
              <p className="text-sm text-gray-600">Avg Stress Level</p>
              <p className="text-4xl font-bold text-orange-600 mt-2">{(insights.avgStress * 100).toFixed(0)}%</p>
            </div>
            <div className="card text-center">
            ...
            <div className="flex justify-between items-center h-16">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-lg font-bold ${timer < 10 ? 'text-red-600' : 'text-gray-900'}`}>{timer}s</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-3xl mx-auto px-4 py-8">
          ...
          <div className="card">
            <div className="mb-4 flex justify-between items-center">
              ...
              <span className="text-sm text-gray-600">Question {questionIndex}/{totalQuestions}</span>
            </div>
            <h2 className="text-xl font-bold text-gray-900 mb-6">{currentQuestion.text || 'Loading question...'}</h2>
              ...
              {currentQuestion.options && currentQuestion.options.map((option: string, idx: number) => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-lg border-2 transition-all ${
                    selectedAnswer === option ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-blue-300'
            ...
            <button
              ...
              className={`btn-primary w-full py-4 text-lg ${!selectedAnswer || isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isSubmitting ? '⏳ Submitting...' : '✓ Submit Answer'}
            </button>
            <p className="text-sm text-gray-500 mt-4 text-center">✨ Next question difficulty will adapt based on your performance</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="card mb-6">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setShowJoinClass(true)} className="btn-primary">+ Join Class</button>
          </div>
          {showJoinClass && (
              ...
              {classes.map((cls: any) => (
                <div
                  ...
                  className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${
                    selectedClass?.id === cls.id ? 'border-green-600 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-green-300'
                  }`}
                >
              ...
              <div className="space-y-4">
                {tests.map((t: any) => (
                  <div key={t.id} className="flex items-center justify-between p-6 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    ...
                    <button onClick={() => startTest(t.id)} className="btn-primary">Start Test →</button>
                  </div>
                ))}
              </div>
            )}
          ...
          {materials.length === 0 ? (
            ...
            <div className="space-y-3">
              {materials.map((material: any) => (
                <div key={material._id} className="border p-4 rounded-lg flex justify-between items-center">
                  <div>
                    <p className="font-semibold">{material.fileName}</p>
                  </div>
                  <a href={`http://localhost:4000/uploads/${material.filePath}`} target="_blank" rel="noopener noreferrer" className="btn-primary" download>
                    Download
                  </a>
                </div>
              ))}
        ...
        <div className="card mt-6">
          ...
          <p className="text-black">Total Attempts: {attempts.length}</p>
          {attempts.map((attempt: any) => (
            <div key={attempt._id} className="border p-4 rounded-lg mb-4 bg-gray-50">
              <p className="mb-2"><strong>Test ID:</strong> {attempt.testId}</p>
              <p className="mb-2"><strong>Completed:</strong> {attempt.completed ? 'Yes' : 'No'}</p>
              <p className="mb-2"><strong>Questions Attempted:</strong> {attempt.results.length}</p>
              <p><strong>Difficulty:</strong> {attempt.currentDifficulty}</p>
            </div>
          ))}
        </div>
      </div>
